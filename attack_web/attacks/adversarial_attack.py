...
class AdversarialAttack:
    ...
    def jsma(self, index=100, ys_target=2, theta=1.0, gamma=0.1):
        ...
        return {
            'adversarial_image': perturbed_image_1.squeeze(),
            'original_label': label,
            'attacked_label': perturbed_label[0]
        }
    ...
    def deepfool(self, index=100, max_iter=10, overshoot=0.8):
        ...
        return {
            'adversarial_image': pert_image.squeeze(),
            'original_label': int(orig_label),
            'attacked_label': int(torch.argmax(fs)),
    ...
    def pgd(self, index=100, epsilon=0.2, iter_eps=0.01, nb_iter=40, clip_min=0.0, clip_max=1.0, C=0.0, ord=np.inf,
        ...
        return {
            'adversarial_image': adv_image_1.squeeze(),
            'original_label': label,
